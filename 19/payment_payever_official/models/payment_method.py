...
class PaymentMethodPayever(models.Model):
    ...
    def _get_compatible_payment_methods(
        ...
        declined_methods = methods.filtered(lambda method: method.id in declined_ids)
        methods -= declined_methods
        payment_utils.add_to_report(
            report,
            declined_methods,
            available=False,
            reason=self.env._('Declined by payever earlier in this checkout.'),
        )
        unfiltered_methods = methods
        methods = methods.filtered(
            lambda method: method._payever_is_available(
        ...
        payment_utils.add_to_report(
            ...
            available=False,
            reason=self.env._(
                'Not available through payever for this customer, country, or order amount.'
            ),
        )
        return methods
